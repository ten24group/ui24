...
export const Header = React.memo(() => {
  ...
  // Memoize menu item separation and formatting to avoid recreating arrays on every render.
  // Menu config is static — only changes on config reload, not on navigation.
  const { menuItems, secondaryMenuItems } = useMemo(() => {
    const primary: any[] = [];
    const secondary: any[] = [];
    ...
    return { menuItems: formatMenuItems(primary), secondaryMenuItems: secondary };
  }, [ actualMenuRecords ]);
